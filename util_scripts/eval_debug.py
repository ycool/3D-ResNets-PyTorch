...
def evaluate(ground_truth_path, result_path, subset, top_k):
    ...
    for line in ground_truth:
        if line[1] in result[line[0]]:
            ...
            wrong_list.append((line[0], line[1], result[line[0]][0]))
    ...
    print('correct num: {} '.format(len(correct_list)))
    print('random {} correct result (video_id, correct_label)'.format(top_k))
    print(random.sample(correct_list, top_k))
    print('wrong num: {} '.format(len(wrong_list)))
    print('random {} wrong result (video_id, correct_label, wrong_result)'.format(top_k))
    print(random.sample(wrong_list, top_k))
    return accuracy
